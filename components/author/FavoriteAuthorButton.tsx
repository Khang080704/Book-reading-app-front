...
import {
  addFavoriteAuthorAction,
  removeFavoriteAuthorAction,
...
export default function FavoriteAuthorButton({
  ...
  const handleToggle = () => {
    ...
    startTransition(async () => {
      const result = prev
        ? await removeFavoriteAuthorAction(authorKey)
        : await addFavoriteAuthorAction(authorKey);
      ...
      if (!result.success) {
        ...
        toast.success(
          prev
            ? "Đã xóa tác giả khỏi danh sách yêu thích"
            : "Đã thêm tác giả vào danh sách yêu thích"
        );
      }
    });
  };
